        ...
        if(splittedBuilding[1]==splittedPlacemark[1] && splittedBuilding[2]==splittedPlacemark[2] )
        {
            for(var k=0;k<reportObj[j].size();k++){
                ...
                if(reportObj[j][k].status =="Pending")
                    $('test').innerHTML+= '<p>Title: </p><p>' + reportObj[j][k].title +'</p><p>Date: </p><p>' + d.getDate() + "-"+(d.getMonth()+1)+"-"+d.getFullYear()+'</p><p>' + '<img width=\"240px\" src="/ProjectAmity/indoorreportimages/'+ reportObj[j][k].image+'"  </a>' +'</p><p>Description: </p><p>' + reportObj[j][k].description+'</p><p>Status:</p><p>' + reportObj[j][k].status +'</p><a href=\'building/individual?id='+reportObj[j][k].id+'\'>Permalink</a></br><center>---</center>'
                else
                    $('test').innerHTML+= '<p>Title: </p><p>' + reportObj[j][k].title +'</p><p>Date: </p><p>' + d.getDate() + "-"+(d.getMonth()+1)+"-"+d.getFullYear()+'</p><p>' + '<img width=\"240px\" src="/ProjectAmity/indoorreportimages/'+ reportObj[j][k].image+'"  </a>' +'</p><p>Description: </p><p>' + reportObj[j][k].description+'</p><p>Status:</p><p><b>' + reportObj[j][k].status +'</b></p>'+ '<p>Official Feedback</p><p>' + '<img width=\"240px\" src="/ProjectAmity/indoorreportimages/'+ reportObj[j][k].image+'"  </a>' +'</p><p></p><p>' + reportObj[j][k].resolvedDescription+'</p>'+'<a href=\'building/individual?id='+reportObj[j][k].id+'\'>Permalink</a></br><center>---</center>'
            }
        }
    }
